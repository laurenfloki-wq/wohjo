...
| OPS_ALERT_PHONE                                                                                                                            | out-of-band ops SMS escalation                                    | E.164 mobile for critical-RED escalation (dispatchOpsAlert). Optional — unset disables the SMS channel (logged loud); email still fires.                                              |
...
| VERIFY_REQUIRE_ACTION_TOKEN                                                                                                                | supervisor verify approve/dispute hardening (`src/lib/verify/action-token.ts`) | Fail-open default (off). When `true`, approve/dispute require the page-minted short-lived action token (signing key derived from `CRON_SECRET`); kills stale-link replay. Not a secret value — a feature flag. |
