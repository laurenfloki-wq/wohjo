...
async function main() {
  ...
  if (sealCount === 0) {
    console.log(
      '  (no SealChip on this page — likely no sealed shifts in the dev DB; skipping chip-pair section)',
    );
  }
  ...
  const supComputed =
    sealCount > 0
      ? await readComputed(page, supChip, [
          'height',
          'padding-top',
          'padding-bottom',
          'padding-left',
  ...
  const sealComputed =
    sealCount > 0
      ? await readComputed(page, sealChip, [
          'height',
          'padding-top',
          'padding-bottom',
          'padding-left',
  ...
  if (supRect && sealRect) {
    ...
    console.log(
      `  baseline of pair: ${round(Math.min(supRect.top, sealRect.top))} -> ${round(Math.max(supRect.bottom, sealRect.bottom))}`,
    );
  } else {
    console.log(
      '  (chips not measurable — no sealed shifts in dev DB; this section was skipped, not failed)',
    );
  }
  ...
  const adjustRect = (await adjustApproveBtn.count())
    ? await readRect(page, adjustApproveBtn)
    : null;
  ...
  const cancelDispRect =
    (await cancelBtns.count()) > 1 ? await readRect(page, cancelBtns.last()) : null;
  ...
  console.log('\n-- DELTAS --');
  if (flagRect && adjustRect)
    console.log(`  flag height − adjust&approve: ${round(flagRect.height - adjustRect.height)} px`);
  if (flagRect && cancelRect)
    console.log(`  flag height − cancel:         ${round(flagRect.height - cancelRect.height)} px`);
  if (adjustRect && approveRect)
    console.log(
      `  adjust − final approve:       ${round(adjustRect.height - approveRect.height)} px`,
    );
  ...
  const tbComputed = await readComputed(page, titleBlock, [
    'height',
    'margin-top',
    'padding-top',
    'padding-bottom',
  ...
  const actionsComputed = await readComputed(page, actionsCluster, [
    'height',
    'align-self',
    'margin-top',
  ]);
  ...
  const headerComputed = await readComputed(page, headerEl, [
    'display',
    'align-items',
    'flex-direction',
    'height',
  ...
  const evidenceFetch = page
    .waitForResponse((r) => r.url().includes('/api/command/super-evidence'), { timeout: 20000 })
    .catch(() => null);
  ...
  if (dlExists) {
    ...
    const rows = await dl.evaluate((node) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        out.push({
          ...
          dtComputed: (() => {
            const s = getComputedStyle(dtEl);
            return { textAlign: s.textAlign, fontSize: s.fontSize };
          })(),
          ddComputed: (() => {
            const s = getComputedStyle(ddEl);
            return { textAlign: s.textAlign, fontSize: s.fontSize };
          })(),
        });
      }
    ...
    console.log(
      `  text-gap range: ${minTextGap}..${maxTextGap}  (spread ${round(maxTextGap - minTextGap)} px)`,
    );
    if (rows.length > 0) {
  ...
  if ((await firstTd.count()) > 0) {
    ...
    console.log(
      `   first-cell text edge: ${tdTextEdge ? JSON.stringify({ left: round(tdTextEdge.left), top: round(tdTextEdge.top) }) : 'null'}`,
    );
  }
  ...
  if (summaryContentLeftX !== null && firstThRect) {
    ...
    console.log(
      `  delta (table − summary):                  ${round(tableContentLeftX - summaryContentLeftX)} px`,
    );
  }
  ...
  if ((await emptyTd.count()) > 0) {
    ...
    if (eRect && tbodyRect) {
      ...
      console.log(
        `   empty centre y: ${round(empCentre)}  | body centre y: ${round(bodyCentre)}  | delta: ${round(empCentre - bodyCentre)} px`,
      );
    }
  } else {
