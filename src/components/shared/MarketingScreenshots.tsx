    ...
    () => {
      ...
      mm.add(
        ...
        (ctx) => {
          ...
          // Seal 3D baseline — hovering above the page, tilted off-axis.
          // The terminal rotation `-8` matches the inline static rubber-
          // stamp tilt, so the reduced-motion baseline and the animated
          // end-state are visually identical.
          if (seal) {
            gsap.set(seal, {
              z: 120,
              rotationX: 15,
              ...
              filter: 'drop-shadow(0 30px 18px rgba(15,15,16,0.32))',
            });
          }
          ...
          // CSS 3D seal press — sharp, decisive, the feel of a
          // notary's stamp coming down on the receipt. The seal
          // starts at z:120 (above the page), rotated slightly off-
          // axis, and presses to z:-2 (a 1–2px overshoot through the
          // page) before settling at z:0. Drop-shadow shrinks as the
          // seal approaches the page so it reads as physically above
          // during the press. No bounce easing — power3.in on the
          // press for the mechanical decisive feel.
          tl.to(
            seal,
            {
              opacity: 0.95,
              ...
              duration: 0.32,
              ease: 'power3.in',
              filter: 'drop-shadow(0 3px 4px rgba(15,15,16,0.28))',
            },
            1.4,
          )
            .to(
              seal,
              {
                ...
                duration: 0.12,
                ease: 'power2.out',
                filter: 'drop-shadow(0 2px 3px rgba(15,15,16,0.22))',
              },
              '>',
            )
    ...
    <PhoneFrame>
      ...
      >
        {/* Serrated white "ticket" with content overlay.
          perspective + preserve-3d so the seal can press onto the
          ticket in 3D space. 900px chosen by feel for a 64px element
          at reading distance — large enough that the press reads as
          a translation through space, small enough that the
          rotateX/Y entrance doesn't distort the geometry beyond a
          notary's-stamp feel. */}
        <div
          style={{
            position: 'relative',
        ...
        >
          {/* Amber rubber-stamp seal (top-right, slight tilt).
            Inline transform/opacity here is the reduced-motion
            static baseline — the seal must read as already stamped
            with no animation. The full/mobile tiers' GSAP timeline
            overrides this with a 3D press entrance (see useGSAP
            block above). */}
          <div
            data-anim="seal"
            style={{
