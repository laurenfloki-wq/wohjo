...
export function GuillocheBand({
  seed, width, height, opacity = 0.06, stroke = 'var(--ink)',
}: { seed: string | null | undefined; width: number; height: number; opacity?: number; stroke?: string }) {
  const id = useId();
  ...
  return (
    ...
    >
      ...
      <g mask={`url(#flos-gc-band-mask-${id})`}>
        <g transform={`translate(${width / 2}, ${height / 2}) scale(1 ${sy}) translate(${-baseSize / 2}, ${-baseSize / 2})`}>
          <path
            d={path}
            fill="none"
            stroke={stroke}
