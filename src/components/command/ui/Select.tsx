...
export function Select<T extends string = string>({
  value, onChange, options, ariaLabel, labelledBy, placeholder = 'Select…',
  disabled, size = 'md', fullWidth = true, style,
}: Props<T>) {
  ...
  useEffect(() => {
    if (!open) return;
    const el = listRef.current?.querySelector<HTMLElement>(`#${CSS.escape(optionIds[activeIndex] ?? '')}`);
  ...
  const onListKeyDown = useCallback((e: React.KeyboardEvent<HTMLUListElement>) => {
    ...
    if (e.key.length === 1) {
      ...
      const hit = options.findIndex((o) =>
        typeof o.label === 'string' && o.label.toLowerCase().startsWith(text),
      );
    ...
    <div style={{ position: 'relative', display: fullWidth ? 'block' : 'inline-block' }}>
      ...
      {open ? (
        <ul
          ...
          style={{
            position: 'absolute', left: 0, right: fullWidth ? 0 : undefined,
            top: 'calc(100% + 4px)',
            background: 'var(--surface)',
            color: 'var(--ink)',
            ...
            return (
              ...
              >
                ...
                <span style={{ minWidth: 0 }}>
                  ...
                  {opt.description ? (
                    <span style={{ display: 'block', color: 'var(--ink-muted)', fontSize: 'var(--t-xs)', marginTop: 2 }}>
                      {opt.description}
                    </span>
                  ) : null}
                </span>
