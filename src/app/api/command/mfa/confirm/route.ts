...
export async function POST(request: Request) {
  const log = routeLogger('POST /api/command/mfa/confirm', request.headers.get('x-request-id'));
  ...
  try {
    ...
    const rl = await checkRateLimitDurable(`admin-mfa-confirm:${membership.userId}`, {
      windowMs: 10 * 60 * 1000,
      maxRequests: 10,
    });
    if (!rl.allowed) {
      log.warn({ userId: membership.userId }, 'admin.mfa.confirm.rate_limited');
    ...
    const r = await confirmEnrolment(log, membership.userId, code, {
