...
interface SupervisorRow {
  ...
  // Patch 5.4 — column renamed in Migration 2.0 from last_batch_sms_date
  // (DATE) to last_batch_sms_sent_at (TIMESTAMPTZ). CRACK 11/67/98 closure.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const supervisor of supervisors as SupervisorRow[]) {
      ...
      // wrote to `last_batch_sms_sent_at` (column didn't exist). Post
      // Migration 2.0 the column is the canonical TIMESTAMPTZ name.
