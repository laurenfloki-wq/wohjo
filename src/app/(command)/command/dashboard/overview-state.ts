...
function workerName(w: { first_name?: string | null; last_name?: string | null; employee_id?: string | null }): string {
...
export async function loadOverviewState(
  ...
  const [
    ...
    supabase.from('sites').select('id', { count: 'exact', head: true }).eq('company_id', companyId).eq('is_active', true),
    supabase
      .from('shifts')
  ...
  const workerIds = new Set<string>([
    ...((pendingSupervisor.data ?? []) as Array<{ worker_id?: string | null }>).map((r) => r.worker_id ?? ''),
    ...((pendingPayroll.data ?? []) as Array<{ worker_id?: string | null }>).map((r) => r.worker_id ?? ''),
    ...((flagged.data ?? []) as Array<{ worker_id?: string | null }>).map((r) => r.worker_id ?? ''),
    ...((disputed.data ?? []) as Array<{ worker_id?: string | null }>).map((r) => r.worker_id ?? ''),
    ...((liveShifts.data ?? []) as Array<{ worker_id?: string | null }>).map((r) => r.worker_id ?? ''),
  ].filter(Boolean));
  const siteIds = new Set<string>([
    ...((pendingSupervisor.data ?? []) as Array<{ site_id?: string | null }>).map((r) => r.site_id ?? ''),
    ...((pendingPayroll.data ?? []) as Array<{ site_id?: string | null }>).map((r) => r.site_id ?? ''),
    ...((flagged.data ?? []) as Array<{ site_id?: string | null }>).map((r) => r.site_id ?? ''),
    ...((disputed.data ?? []) as Array<{ site_id?: string | null }>).map((r) => r.site_id ?? ''),
    ...((liveShifts.data ?? []) as Array<{ site_id?: string | null }>).map((r) => r.site_id ?? ''),
  ].filter(Boolean));

  const workersById: Record<string, { first_name?: string | null; last_name?: string | null; employee_id?: string | null }> = {};
  if (workerIds.size > 0) {
    const { data: ws } = await supabase
      .from('workers')
      .select('id, first_name, last_name, employee_id')
      .in('id', Array.from(workerIds));
    (ws ?? []).forEach((w: { id: string; first_name?: string | null; last_name?: string | null; employee_id?: string | null }) => { workersById[w.id] = w; });
  }
  const sitesById: Record<string, { name?: string | null }> = {};
  if (siteIds.size > 0) {
    const { data: ss } = await supabase.from('sites').select('id, name').in('id', Array.from(siteIds));
    (ss ?? []).forEach((s: { id: string; name?: string | null }) => { sitesById[s.id] = s; });
  }
  ...
  function isFlagged(row: { anomaly_flags?: unknown }): boolean {
    ...
    return flags.some((f) => typeof f === 'object' && f !== null && 'severity' in f && (f as { severity?: string }).severity === 'HIGH');
  }

  const needs_attention: NeedsAttentionItem[] = [
    ...((flagged.data ?? []) as Array<{ id: string; worker_id?: string | null; site_id?: string | null; total_hours?: string | null; shift_date: string; anomaly_flags?: unknown }>).map((s) => ({
      shift_id: s.id,
    ...
    ...((disputed.data ?? []) as Array<{ id: string; worker_id?: string | null; site_id?: string | null; total_hours?: string | null; shift_date: string }>).map((s) => ({
      shift_id: s.id,
      worker_name: nameForWorker(s.worker_id),
    ...
    ...((pendingPayroll.data ?? []) as Array<{ id: string; worker_id?: string | null; site_id?: string | null; total_hours?: string | null; shift_date: string; anomaly_flags?: unknown }>).map((s) => ({
      shift_id: s.id,
      worker_name: nameForWorker(s.worker_id),
      reason: 'pending_payroll' as const,
      reason_label: isFlagged(s) ? 'Pending final approval — review the flags' : 'Pending final approval',
      hours: parseFloat(s.total_hours ?? '0'),
      site_name: nameForSite(s.site_id),
      shift_date: s.shift_date,
      href: '/command/approvals',
    })),
    ...((pendingSupervisor.data ?? []) as Array<{ id: string; worker_id?: string | null; site_id?: string | null; total_hours?: string | null; shift_date: string }>).map((s) => ({
      shift_id: s.id,
      worker_name: nameForWorker(s.worker_id),
  ...
  const export_blockers: ExportReadinessBlocker[] = ((workersMissingMyob.data ?? []) as Array<{ id: string; first_name?: string | null; last_name?: string | null; employee_id?: string | null }>).map((w) => ({
    worker_id: w.id,
  ...
  const periodShifts = (shiftsThisPeriod.data ?? []) as Array<{
    id: string; total_hours: string | null; status: string; worker_id: string | null;
  }>;
  ...
  const week_hours_verified = verified.reduce((sum, s) => sum + parseFloat(s.total_hours ?? '0'), 0);
  ...
  const live_shifts: LiveShift[] = ((liveShifts.data ?? []) as Array<{ id: string; worker_id?: string | null; site_id?: string | null; start_time?: string | null }>).map((s) => ({
    shift_id: s.id,
    worker_name: nameForWorker(s.worker_id),
    site_name: nameForSite(s.site_id),
