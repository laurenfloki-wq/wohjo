-- CRACK 232 v2 — fix in-batch dedup detection.
-- Original v1 used `(SELECT ... GROUP BY 1 HAVING COUNT(*) > 1 LIMIT 1) IS NOT NULL`
-- which trips a 42803 aggregate-in-GROUP-BY restriction at parse time on
-- some PG versions. Replaced with a length-vs-cardinality comparison
-- which is simpler and version-portable.
